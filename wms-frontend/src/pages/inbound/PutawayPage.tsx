                  ...
                  <Td className="text-xs">
                    {t.suggested_location_id ? (
                      <span className="inline-flex items-center gap-1 text-primary-700">
                        <ArrowRight className="h-3 w-3" /> {t.suggested_location_id.slice(0, 8)}
                      </span>
        ...
        <div className="space-y-4">
          {target?.suggested_location_id && (
            <div className="rounded-lg bg-primary-50 border border-primary-100 px-3 py-2 text-sm text-primary-800">
              Ubicación sugerida: <span className="font-mono">{target.suggested_location_id}</span>
            </div>
          )}
          <Input label="Ubicación real (Location ID)" value={actualLocation}
            onChange={e => setActualLocation(e.target.value)}
            placeholder="UUID de la ubicación de destino" />
          {isOverride && (
